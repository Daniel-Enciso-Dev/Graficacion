...
function drawStarRotated(){
    ...
    // rotación (simulación eje X → aquí será rotación 2D)
    ctx.rotate(angleX);

    // dibujar estrella centrada en (0,0)
    for (let i = 10; i <= 300; i += 10) {

        drawLine(i, 0, 0, 300 - i);
        drawLine(-i, 0, 0, 300 - i);
...
function rotateStarStep(){
    ...
    // aumentar 5 grados (convertido a radianes)
    angleX += 5 * Math.PI / 180;

    // limpiar pantalla SIN detener nada
    ctx.clearRect(0,0,canvas.width,canvas.height);

    // redibujar plano
    drawCartesianPlane();

    // dibujar estrella rotada
    drawStarRotated();
}
